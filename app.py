...
def clean_text(val):
    """
    Prevent issues with special characters like &
    """
    if pd.isna(val):
        return ""
...
def build_full_division(division, division_description):
    division = clean_text(division)
    division_description = clean_text(division_description)

    if division and division_description:
        return f"Division {division} - {division_description}"
...
if uploaded_templates:
    ...
    for uploaded_file in uploaded_templates:
        ...
        if os.path.exists(file_path):
            st.sidebar.warning(f"{uploaded_file.name} already exists and was overwritten.")

        with open(file_path, "wb") as f:
            f.write(uploaded_file.getbuffer())

    st.sidebar.success("Templates saved successfully!")
...
if option == "Manual Form":
    ...
    with col2:
        ...
        data["Division"] = st.text_input("Division Number")

        data["Division Description"] = st.text_input("Division Description")
    ...
    with tab2:
        ...
        with zipfile.ZipFile(zip_buffer, "w") as zip_file:
            ...
            for i, record in enumerate(records):
                ...
                payout_words = build_payout_words(
                    payout_value
                )
                ...
                full_division = build_full_division(
                    division,
...
Project: {get_val(record, "Project Name", "ProjectName")}
        ...
        with zipfile.ZipFile(zip_buffer, "w") as zip_file:
            ...
            for i, record in enumerate(records):
                ...
                context = {
                    ...
                    # ---------------- SCOPE ----------------
                    "Scope":
                        get_val(record, "Scope"),

                    "Addendums":
                        get_val(record, "Addendums"),

                    "alternate":
                        get_val(record, "alternate"),
                ...
                for template_name in selected_templates:
                    ...
                    company = clean_filename(company)
